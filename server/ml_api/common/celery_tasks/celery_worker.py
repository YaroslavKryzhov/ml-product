...
class BackgroundTask(Task):
    ...
    def after_return(self, status, retval, task_id, args, kwargs, einfo):
        pub_sub.publish_to_channel(
            task_id=task_id, status=status, message=retval)


@app_celery.task(base=BackgroundTask, name="apply_function")
...
@app_celery.task(base=BackgroundTask, name="copy_pipeline")
