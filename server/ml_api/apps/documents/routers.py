...
from ml_api.apps.documents.schemas import DocumentDB, ColumnMarks, AvailableFunctions
...
def apply_pipeline_to_csv(document_from: str, document_to: str, db: get_db = Depends(), user: User = Depends(current_active_user)):
...
# def standartize_features(filename: str, db: get_db = Depends(), user: User = Depends(current_active_user)):
#     DocumentService(db, user).standardize_features(filename)
#     return {"filename": filename}
#
#
# @documents_method_router.put("/fs_select_k_best")
# def fs_select_k_best(filename: str, db: get_db = Depends(), user: User = Depends(current_active_user)):
#     DocumentService(db, user).fs_select_k_best(filename)
#     return {"filename": filename}
#
#
# @documents_method_router.put("/outlier_three_sigma")
# def outlier_three_sigma(filename: str, db: get_db = Depends(), user: User = Depends(current_active_user)):
#     DocumentService(db, user).outlier_three_sigma(filename)
#     return {"filename": filename}
#
#
# @documents_method_router.put("/miss_insert_mean_mode")
# def miss_insert_mean_mode(filename: str, db: get_db = Depends(), user: User = Depends(current_active_user)):
