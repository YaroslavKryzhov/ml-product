...
from ml_api.apps.documents.schemas import DocumentFullInfo, DocumentDescription
...
def rename_document(filename: str,
                    new_filename: str,
                    db: get_db = Depends(),
                    user: User = Depends(current_active_user)):
...
def read_all_user_documents(db: get_db = Depends(),
                            user: User = Depends(current_active_user)):
...
def read_document_info(filename: str,
                       db: get_db = Depends(),
                       user: User = Depends(current_active_user)):
...
def read_document_with_pagination(filename: str,
                                  page: int = 1,
                                  db: get_db = Depends(),
                                  user: User = Depends(current_active_user)):
    result = DocumentService(db, user).read_document_with_pagination(filename,
                                                                     page)
    return result


@documents_df_router.get("/info", response_model=List[ColumnDescription])
def document_columns_stat_info(filename: str,
                               db: get_db = Depends(),
                               user: User = Depends(current_active_user)):
...
def document_stat_describe(filename: str,
                           db: get_db = Depends(),
                           user: User = Depends(current_active_user)):
...
def set_column_as_categorical(filename: str,
                              ...
                              db: get_db = Depends(),
                              user: User = Depends(current_active_user)):
    result = DocumentService(db, user).set_column_as_categorical(filename,
                                                                 column_name)
    if result:
        return ServiceResponse(status_code=status.HTTP_200_OK,
...
def set_column_as_numeric(filename: str,
                          ...
                          db: get_db = Depends(),
                          user: User = Depends(current_active_user)):
    result = DocumentService(db, user).set_column_as_numeric(filename,
                                                             column_name)
    if result:
        return ServiceResponse(status_code=status.HTTP_200_OK,
...
def delete_column(filename: str,
                  ...
                  db: get_db = Depends(),
                  user: User = Depends(current_active_user)):
    result = DocumentService(db, user).apply_function(filename,
                                                      'drop_column',
                                                      column_name)
    if result:
        return ServiceResponse(status_code=status.HTTP_200_OK,
