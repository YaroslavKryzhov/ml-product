...
class AvailableFunctions(Enum):
    ...
    miss_insert_mean_mode = 'miss_insert_mean_mode'  # Замена пропусков: Среднее и мода
    miss_linear_imputer = 'miss_linear_imputer'  # Замена пропусков: Линейная модель
    miss_knn_imputer = 'miss_knn_imputer'  # Замена пропусков: К-ближних соседей
    # GROUP 2: Трансформация признаков
    standardize_features = 'standardize_features'  # Стандартизация цисленных признаков
    ...
    outliers_isolation_forest = 'outliers_isolation_forest'  # Удаление выбросов: IsolationForest
    outliers_elliptic_envelope = 'outliers_elliptic_envelope'  # Удаление выбросов: EllipticEnvelope
    outliers_local_factor = 'outliers_local_factor'  # Удаление выбросов: LocalOutlierFactor
    outliers_one_class_svm = 'outliers_one_class_svm'  # Удаление выбросов: OneClassSVM
    outliers_sgd_one_class_svm = 'outliers_sgd_one_class_svm'  # Удаление выбросов: SGDOneClassSVM
    # GROUP 4: Отбор признаков
    fs_select_percentile = 'fs_select_percentile'  # Отбор признаков: по перцентилю
    ...
    fs_select_from_model = 'fs_select_from_model'  # Отбор признаков: из линейной модели
