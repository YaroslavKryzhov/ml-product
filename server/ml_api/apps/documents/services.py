...
class DocumentService:
    ...
    def read_all_documents_info(self):
        documents = DocumentPostgreCRUD(self._db, self._user).read_all_documents_by_user()
        return documents

    def read_document_with_pagination(self, filename: str, page: int = 1, rows_on_page: int = 50) -> pd.DataFrame:
        if self.check_if_document_name_exists(filename):
    ...
    def get_column_stat_description(self, filename: str, bins: int = 10) -> [str, str]:
        if DocumentPostgreCRUD(self._db, self._user).read_document_by_name(filename=filename) is not None:
            ...
            task_type = column_marks['task_type']
    ...
    def update_pipeline(self, filename: str, function_name: str, param: Union[int, float, str] = None):
        ...
        if param:
            pipeline.append([function_name, param])
        else:
    ...
    def apply_function(self, filename: str, function_name: str, param: Union[int, float, str] = None) -> Union[List, bool]:
