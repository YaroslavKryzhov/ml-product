...
from ml_api.apps.documents.repository import (
    DocumentFileCRUD,
    DocumentPostgreCRUD,
)
from ml_api.apps.documents.schemas import (
    DocumentShortInfo,
    DocumentFullInfo,
    TaskType,
    PipelineElement,
...
class DocumentService:
    ...
    def read_document_with_pagination(
        self, filename: str, page: int = 1, rows_on_page: int = 50
    ) -> ReadDocumentResponse:
        if self._check_if_document_name_exists(filename):
            df = self._read_document(filename)
            length = len(df)
    ...
    @staticmethod
    def _create_hist_data(
        df: pd.DataFrame, column_name: str, bins: int
    ) -> ColumnDescription:
        ints = (
    ...
    def create_column_stats(
        ...
        if target_name:
            ...
            if task_type == 'regression':
                result.append(
                    self._create_hist_data(
                        df=df, column_name=target_name, bins=bins
                    )
                )
            elif task_type == 'classification':
                result.append(
                    self._create_counts_data(df=df, column_name=target_name)
                )
        return result

    def get_document_columns_info(
        self, filename: str, bins: int = 10
    ) -> List[ColumnDescription]:
        if (
            DocumentPostgreCRUD(self._db, self._user).read_by_name(
                filename=filename
            )
            is not None
        ):
    ...
    def get_document_stat_description(
        self, filename: str
    ) -> DocumentDescription:
        if self._check_if_document_name_exists(filename):
            df = self._read_document(filename)
    ...
    def _validate_column_types(
        self, filename: str, target_column: str, task_type: TaskType
    ) -> ColumnTypes:
        df = self._read_document(filename)
    ...
    def set_column_types(
        ...
        column_types = self._validate_column_types(
            filename, target_column, task_type
        )
        self.update_column_types(filename, column_types)

    def read_column_types(self, filename: str) -> ColumnTypes:
        column_types = (
            DocumentPostgreCRUD(self._db, self._user)
    ...
    def update_pipeline(
        ...
        if param:
            pipeline.append(
                PipelineElement(function_name=function_name, param=param)
            )
        else:
    ...
    def apply_pipeline_to_csv(
        self, filename: str, pipeline: List[PipelineElement]
    ):
        # to do: find errors
        for function in pipeline:
            if function.param:
                ...
                self.apply_function(
                    filename=filename, function_name=function.function_name
                )
    ...
    def apply_function(
        ...
        document_operator.apply_function(
            function_name=function_name, param=param
        )
        ...
        if document_operator.is_pipelined():
            self.update_pipeline(
                filename, function_name=function_name, param=param
            )
        self.update_column_types(
            filename, column_types=document_operator.get_column_types()
        )
        DocumentFileCRUD(self._user).update(
            filename, document_operator.get_df()
        )
        self._update_change_date_in_db(filename)
        return True
...
class DocumentOperator:
    ...
    def apply_function(
        self, function_name: str, param: Union[int, float, str] = None
    ):
