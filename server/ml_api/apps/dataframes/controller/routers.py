...
def upload_document(
    ...
    result = DocumentService(db, user).upload_document_to_db(
...
def delete_document(
    ...
    if result:
        ...
        return ServiceResponse(
            status_code=status.HTTP_409_CONFLICT, content="Error with deletion"
        )
...
def read_document_with_pagination(
    ...
    result = DocumentService(db, user).read_document_with_pagination(
        filename, page
    )
    return result
...
def set_column_as_categorical(
    ...
    result = DocumentService(db, user).set_column_as_categorical(
...
def set_column_as_numeric(
    ...
    result = DocumentService(db, user).set_column_as_numeric(
        filename, column_name
    )
    if result:
        return ServiceResponse(
...
def delete_column(
    ...
    result = DocumentService(db, user).apply_function(
        filename, 'drop_column', column_name
    )
    if result:
        return ServiceResponse(
            status_code=status.HTTP_200_OK,
