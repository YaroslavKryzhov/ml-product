...
class ModelService:
    ...
    def read_model_info(self, model_name: str) -> CompositionFullInfoResponse:
        ...
        model_info = CompositionFullInfoResponse(
            csv_name=csv_name, **model_info.dict()
        )
        return model_info
    ...
    def read_models_info(self) -> List[CompositionShortInfoResponse]:
        ...
        for model in models:
            ...
            result.append(
                CompositionShortInfoResponse(csv_name=csv_name, **model.dict())
            )
        return models
    ...
    def train_model(
        ...
        document_id = (
            DocumentService(self._db, self._user)
            .read_document_info(filename=document_name)
            .id
        )
        ...
        target_column = (
            DocumentService(self._db, self._user)
            .read_column_types(document_name)
            .target
        )
        ...
        # create model in db
        ModelPostgreCRUD(self._db, self._user).create(
            model_name=model_name,
            csv_id=str(document_id),
            task_type=task_type,
    ...
    def _validate_training(
        ...
        composition = CompositionConstructor(
            task_type=task_type,
            composition_type=composition_type,
            models_with_params=composition_params,
        ).build_composition()

        model, metrics = CompositionValidator(
            task_type=task_type,
        ...
        report = CompositionReport(csv_name=document_name, metrics=metrics)
    ...
    def _save_model(
        ...
        ModelPostgreCRUD(self._db, self._user).update(
...
class ModelConstructor:
    ...
    def _construct_classification_model(self):
        ...
        if self.model_type.value == 'MLPClassifier':
            ...
            return model
        if self.model_type.value == 'KNeighborsClassifier':
            model = self._get_k_neighbors_classifier()
            return model
        return None
    ...
    def _get_gradient_boosting_classifier(self):
        params = models_config.GradientBoostingClassifierParameters(
            **self.params
        )
    ...
    def _get_k_neighbors_classifier(self):
        params = models_config.KNeighborsClassifierParameters(**self.params)
        model = KNeighborsClassifier(**params.dict())
        return model
...
class CompositionConstructor:
    ...
    def build_composition(self):
        ...
        if self.composition_type == 'simple_voting':
            ...
            composition = StackingClassifier(
