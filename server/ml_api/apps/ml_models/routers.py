...
def predict(document_name: str, model_name: str, db: get_db = Depends(), user: User = Depends(current_active_user)):
...
def read_model_info(model_name: str, db: get_db = Depends(), user: User = Depends(current_active_user)):
    result = ModelService(db, user).read_model_info(model_name=model_name)
    return result


@models_router.get("/all")
def read_all_user_models(db: get_db = Depends(), user: User = Depends(current_active_user)):
    result = ModelService(db, user).read_models_info()
    return result


@models_router.get("/download")
def download_model(model_name: str, db: get_db = Depends(), user: User = Depends(current_active_user)):
...
def delete_model(model_name: str, db: get_db = Depends(), user: User = Depends(current_active_user)):
