...
from ml_api.apps.ml_models.schemas import AvailableParams, AvailableTaskTypes, \
    AvailableCompositions, CompositionParams, CompositionShortInfoResponse, \
...
def predict(document_name: str, model_name: str,
            db: get_db = Depends(), user: User = Depends(current_active_user)):
    result = ModelService(db, user).predict_on_model(filename=document_name,
                                                     model_name=model_name)
    return result


@models_router.get("/download")
def download_composition(model_name: str, db: get_db = Depends(),
                         user: User = Depends(current_active_user)):
...
def rename_composition(model_name: str, new_model_name: str,
                       db: get_db = Depends(),
                       user: User = Depends(current_active_user)):
...
def delete_composition(model_name: str, db: get_db = Depends(),
                       user: User = Depends(current_active_user)):
...
def read_all_user_compositions(db: get_db = Depends(),
                               user: User = Depends(current_active_user)):
...
def read_composition_info(model_name: str, db: get_db = Depends(),
                          user: User = Depends(current_active_user)):
