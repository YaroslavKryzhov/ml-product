...
class ModelPostgreCRUD(BaseCRUD):
    ...
    # CREATE
    def create(self, model_name: str, csv_id: str, task_type: str,
    ...
    # READ
    def read_by_name(self, model_name: str):
        filepath = self.file_path(model_name)
        model = self.session.query(Model.id, Model.name, Model.create_date,
            Model.task_type, Model.composition_type, Model.hyperparams,
            Model.metrics).filter(Model.filepath == filepath).first()
        if model is not None:
            return model
        return None

    def read_all(self):
        return self.session.query(Model.name, Model.create_date,
            Model.task_type, Model.composition_type).filter(
