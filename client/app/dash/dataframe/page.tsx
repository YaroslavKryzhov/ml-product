...
function getCellColor(n: string) {
    ...
    let res = num > 0 ? 'green' : 'red';
    const abs = Math.abs(num);

    if (abs > 0 && abs < 0.2) res += '.200';
    else if (abs >= 0.2 && abs < 0.4) res += '.300';
    else if (abs >= 0.4 && abs < 0.6) res += '.400';
    else if (abs >= 0.6 && abs < 0.8) res += '.500';
    else if (abs >= 0.8 && abs <= 1) res += '.600';
...
export default function DataframePage() {
    ...
    const view = query.get('view') ?? tableViews[0];
    ...
    return dataframe ? <Suspense fallback={<></>}>
        ...
        <VStack w='100%' borderRadius={6} p='8px 16px 8px 24px' align='start' spacing='24px'>
            <HStack spacing='18px'>
                ...
                <Button onClick={onOpenApply} colorScheme='green' fontSize='14px'>Применить пайплайн</Button>
            </HStack>
        ...
        <VStack w='100%' spacing='14px' align='start' bg='main.100' borderRadius={6} p='14px'>
            <HStack w='100%' spacing='8px'>
                {['Детально', 'Компактно', 'Подробнее', 'Матрица корреляции', 'Отбор признаков'].map((x: string, i: number) => <Link key={i} href={`/dash/dataframe?dataframe_id=${query.get('dataframe_id')}&view=${x}`}>
                    <Text color={view === x ? 'main.400' : 'black'} bg={view === x ? 'main.200' : 'none'} borderRadius={6} p='6px 12px' fontSize='24px' lineHeight='100%' letterSpacing='1px' _hover={{ cursor: 'pointer', bg: 'main.200' }} transition='0.1s'>{x}</Text>
                </Link>)}
            </HStack>
